...
pub trait SledSerde: Serialize + DeserializeOwned {
    ...
    fn de<T: AsRef<[u8]>>(v: T) -> Result<Self, SledStorageError>
    where
        Self: Sized,
    {
    ...
    R: RangeBounds<IVec>,
{
    ...
    fn ser(&self) -> Result<R, SledStorageError>;
...
where
    ...
    V: RangeBounds<SD>,
{
    fn ser(&self) -> Result<(Bound<IVec>, Bound<IVec>), SledStorageError> {
...
fn bound_ser<SD: SledOrderedSerde>(v: Bound<&SD>) -> Result<Bound<sled::IVec>, SledStorageError> {
    let res = match v {
