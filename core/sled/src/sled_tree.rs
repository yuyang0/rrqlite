...
impl SledTree {
    ...
    pub fn txn<T>(
        &self,
        sync: bool,
        f: impl Fn(TransactionSledTree<'_>) -> Result<T, SledStorageError>,
    ) -> Result<T, SledStorageError> {
        let sync = sync && self.sync;

        let result: TransactionResult<T, SledStorageError> = self.tree.transaction(move |tree| {
            ...
            match r {
                ...
                Err(meta_sto_err) => {
                    ...
                    match &meta_sto_err {
                        SledStorageError::BytesError(_e) => {
                            Err(ConflictableTransactionError::Abort(meta_sto_err))
                        }
                        SledStorageError::SerdeError(_e) => {
                            Err(ConflictableTransactionError::Abort(meta_sto_err))
                        }
                        SledStorageError::SledError(_e) => {
                            Err(ConflictableTransactionError::Abort(meta_sto_err))
                        }
                        SledStorageError::TransactionConflict => {
                            Err(ConflictableTransactionError::Conflict)
                        }
                        SledStorageError::AppError(_app_err) => {
                            Err(ConflictableTransactionError::Abort(meta_sto_err))
                        }
                        SledStorageError::SnapshotError(_e) => {
                            Err(ConflictableTransactionError::Abort(meta_sto_err))
                        }
                        SledStorageError::Damaged(_e) => {
                            Err(ConflictableTransactionError::Abort(meta_sto_err))
                        }
                    }
                }
        ...
        match result {
            ...
            Err(txn_err) => match txn_err {
                ...
                TransactionError::Storage(sto_err) => {
                    Err(SledStorageError::SerdeError(sto_err.to_string()))
                }
            },
        }
    }
    ...
    pub fn contains_key<KV: SledKeySpace>(&self, key: &KV::K) -> SledStorageResult<bool>
    where
        KV: SledKeySpace,
    {
        let got = self
    ...
    pub fn get<KV: SledKeySpace>(&self, key: &KV::K) -> SledStorageResult<Option<KV::V>> {
        let got = self
            .tree
    ...
    pub fn last<KV>(&self) -> SledStorageResult<Option<(KV::K, KV::V)>>
    where
        KV: SledKeySpace,
    {
    ...
    pub async fn remove<KV>(&self, key: &KV::K, flush: bool) -> SledStorageResult<Option<KV::V>>
    where
        KV: SledKeySpace,
    {
        let removed = self
    ...
    pub async fn range_remove<KV, R>(&self, range: R, flush: bool) -> SledStorageResult<()>
    where
        KV: SledKeySpace,
        R: RangeBounds<KV::K>,
    {
    ...
    /// Get keys in `range`
    pub fn range_keys<KV, R>(&self, range: R) -> SledStorageResult<Vec<KV::K>>
    where
        KV: SledKeySpace,
        R: RangeBounds<KV::K>,
    {
    ...
    pub fn range_kvs<KV, R>(&self, range: R) -> SledStorageResult<Vec<(KV::K, KV::V)>>
    where
        KV: SledKeySpace,
        R: RangeBounds<KV::K>,
    {
    ...
    pub fn range<KV, R>(
        &self,
        range: R,
    ) -> SledStorageResult<impl DoubleEndedIterator<Item = SledStorageResult<(KV::K, KV::V)>>>
    where
        KV: SledKeySpace,
        R: RangeBounds<KV::K>,
    {
    ...
    pub fn scan_prefix<KV>(&self, prefix: &KV::K) -> SledStorageResult<Vec<(KV::K, KV::V)>>
    where
        KV: SledKeySpace,
    {
        let mut res = vec![];
    ...
    pub fn range_values<KV, R>(&self, range: R) -> SledStorageResult<Vec<KV::V>>
    where
        KV: SledKeySpace,
        R: RangeBounds<KV::K>,
    {
    ...
    pub async fn append<KV>(&self, kvs: &[(KV::K, KV::V)]) -> SledStorageResult<()>
    where
        KV: SledKeySpace,
    {
        let mut batch = sled::Batch::default();
    ...
    async fn insert<KV>(&self, key: &KV::K, value: &KV::V) -> SledStorageResult<Option<KV::V>>
    where
        KV: SledKeySpace,
    {
        let k = KV::serialize_key(key)?;
    ...
    pub async fn insert_value<KV>(&self, value: &KV::V) -> SledStorageResult<Option<KV::V>>
    where
        KV: SledKeySpace,
        KV::V: SledValueToKey<KV::K>,
    {
    ...
    #[tracing::instrument(level = "debug", skip(self))]
    async fn flush_async(&self, flush: bool) -> SledStorageResult<()> {
        if flush && self.sync {
            self.tree
                .flush_async()
                .await
...
impl<'a, KV: SledKeySpace> Store<KV> for AsTxnKeySpace<'a, KV> {
        ...
        F: FnMut(Option<KV::V>) -> Option<KV::V>,
    {
        ...
        let old_val: Result<Option<KV::V>, SledStorageError> = match old_val_ivec {
            Some(v) => Ok(Some(KV::deserialize_value(v)?)),
            None => Ok(None),
        };
...
impl<'a, KV: SledKeySpace> AsKeySpace<'a, KV> {
    pub fn contains_key(&self, key: &KV::K) -> SledStorageResult<bool> {
        self.inner.contains_key::<KV>(key)
    }

    pub async fn update_and_fetch<F>(&self, key: &KV::K, f: F) -> SledStorageResult<Option<KV::V>>
    where
        F: FnMut(Option<KV::V>) -> Option<KV::V>,
    {
    ...
    pub fn get(&self, key: &KV::K) -> SledStorageResult<Option<KV::V>> {
        self.inner.get::<KV>(key)
    }

    pub fn last(&self) -> SledStorageResult<Option<(KV::K, KV::V)>> {
        self.inner.last::<KV>()
    }

    pub async fn remove(&self, key: &KV::K, flush: bool) -> SledStorageResult<Option<KV::V>> {
        self.inner.remove::<KV>(key, flush).await
    }

    pub async fn range_remove<R>(&self, range: R, flush: bool) -> SledStorageResult<()>
    where
        R: RangeBounds<KV::K>,
    {
    ...
    pub fn range_keys<R>(&self, range: R) -> SledStorageResult<Vec<KV::K>>
    where
        R: RangeBounds<KV::K>,
    {
    ...
    pub fn range_kvs<R>(&self, range: R) -> SledStorageResult<Vec<(KV::K, KV::V)>>
    where
        R: RangeBounds<KV::K>,
    {
    ...
    pub fn scan_prefix(&self, prefix: &KV::K) -> SledStorageResult<Vec<(KV::K, KV::V)>> {
        self.inner.scan_prefix::<KV>(prefix)
    }

    pub fn range_values<R>(&self, range: R) -> SledStorageResult<Vec<KV::V>>
    where
        R: RangeBounds<KV::K>,
    {
    ...
    pub async fn append(&self, kvs: &[(KV::K, KV::V)]) -> SledStorageResult<()> {
        self.inner.append::<KV>(kvs).await
    }

    pub async fn append_values(&self, values: &[KV::V]) -> SledStorageResult<()>
    where
        KV::V: SledValueToKey<KV::K>,
    {
    ...
    pub async fn insert(&self, key: &KV::K, value: &KV::V) -> SledStorageResult<Option<KV::V>> {
        self.inner.insert::<KV>(key, value).await
    }

    pub async fn insert_value(&self, value: &KV::V) -> SledStorageResult<Option<KV::V>>
    where
        KV::V: SledValueToKey<KV::K>,
    {
