...
impl RaftState {
    ...
    pub async fn open_create(
        ...
        let (id, is_open) = if let Some(curr_id) = curr_id {
            match (open, create) {
                ...
                (None, Some(_)) => {
                    return Err(StoreError::MetaStoreAlreadyExists(curr_id));
                }
    ...
    #[tracing::instrument(level = "debug", skip(self))]
    pub async fn write_state_machine_id(&self, id: &(u64, u64)) -> StoreResult<()> {
        let state = self.state();
        state
            .insert(
    ...
    #[tracing::instrument(level = "debug", skip(self))]
    pub fn read_state_machine_id(&self) -> StoreResult<(u64, u64)> {
        let state = self.state();
    ...
    #[tracing::instrument(level = "debug", skip(self))]
    pub async fn write_vote(&self, vt: &Vote) -> StoreResult<()> {
        let state = self.state();
        state
            .insert(&RaftStateKey::Vote, &RaftStateValue::Vote(vt.clone()))
            .await?;
        Ok(())
    }

    #[tracing::instrument(level = "debug", skip(self))]
    pub fn read_vote(&self) -> StoreResult<Option<Vote>> {
        let state = self.state();
        let vt = state.get(&RaftStateKey::Vote)?;
        let vt = vt.map(Vote::from);
        Ok(vt)
    }
